...
def fit_power_law(label, frequency_ghz, flux_jy, uncertainty_jy,
    ...
    def model(x, s_ref, alpha):
        return power_law(x, s_ref, alpha, reference_frequency_ghz)
    popt, pcov = curve_fit(
...
def fit_ratio_power_law(numerator, denominator, frequency_ghz, ratio, ratio_error,
    ...
    def model(x, ratio_ref, delta_alpha):
        return ratio_power_law(x, ratio_ref, delta_alpha, reference_frequency_ghz)
