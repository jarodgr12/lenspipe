...
def configure_frequency_frame(limits_ghz, ticks_ghz) -> None:
    """Set the frequency frame and tick marks used by every Stage 3 figure."""
    global SUMMARY_FREQUENCY_LIMITS_GHZ, SUMMARY_FREQUENCY_TICKS_GHZ, SUMMARY_DATA_FREQUENCY_RANGE_GHZ
    low, high = (float(limits_ghz[0]), float(limits_ghz[1]))
    SUMMARY_FREQUENCY_LIMITS_GHZ = (low, high)
    SUMMARY_FREQUENCY_TICKS_GHZ = np.asarray(list(ticks_ghz), dtype=float)
    SUMMARY_DATA_FREQUENCY_RANGE_GHZ = (
        float(SUMMARY_FREQUENCY_TICKS_GHZ.min()) if SUMMARY_FREQUENCY_TICKS_GHZ.size else low,
        float(SUMMARY_FREQUENCY_TICKS_GHZ.max()) if SUMMARY_FREQUENCY_TICKS_GHZ.size else high,
    )


def _resolve_x_limits(x_limits):
...
def configure_figure_formats(formats) -> None:
    """Set which formats every Stage 3 figure is written in (pdf, png, svg)."""
    global FIGURE_FORMATS
    ordered = tuple(dict.fromkeys(str(fmt).lower() for fmt in formats))
    if not ordered:
...
def plot_epoch_normalized_weighted_flux_ratios_vs_mjd(
    ...
    if y_limits is not None:
        ...
        for axis, limits in zip(axes, panel_limits, strict=False):
